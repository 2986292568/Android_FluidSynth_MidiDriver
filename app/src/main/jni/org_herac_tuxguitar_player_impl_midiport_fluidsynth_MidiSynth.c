...
JNIEXPORT void JNICALL Java_org_herac_tuxguitar_player_impl_midiport_fluidsynth_MidiSynth_FillBuffer
(JNIEnv * env, jobject obj, jlong ptr, jshortArray shortarray, jint len){
	...
	if(handle != NULL && handle->synth != NULL){
		...
		jint a = fluid_synth_write_s16 (handle->synth,len/2,arr, 0, 2,arr, 1, 2);
